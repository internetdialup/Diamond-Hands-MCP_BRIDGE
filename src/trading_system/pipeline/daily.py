...
class DailyPipeline:
    ...
    def run(self) -> PipelineResult:
        ...
        # Generate Utilitarian News Feed
        top_12_news = []
        seen_headlines = set()

        def add_news(topic: str, headlines: list[dict], limit: int = 2):
            count = 0
            for article in headlines:
                ...
                title = title.strip()
                # Skip repetitive or generic placeholders
                if not title or title.lower() == 'market update' or title in seen_headlines:
                    continue
                
                display_title = title if len(title) <= 60 else title[:57] + "..."
                top_12_news.append(NewsItem(topic=topic, sentiment_score=0.0, summary=display_title))
                seen_headlines.add(title)
                count += 1
                if count >= limit:
                    break

        # 1. Prioritize Volatility (VIX) News
        add_news(snapshot.volatility_symbol, snapshot.volatility_news, limit=1)
        
        # 2. Inject real headlines from other symbols
        for s_symbol, s_snap in snapshot.symbols.items():
            add_news(s_symbol, s_snap.news, limit=2)
    ...
    def _render_markdown(self, report: DailyReportContract) -> str:
        ...
        if report.top_12_news:
            for news in report.top_12_news:
                lines.append(f"- **{news.topic}**: {news.summary}")
        else:
            lines.append("- No major catalysts detected.")
