...
class YahooFinanceProvider(DataProvider):
    def load_snapshot(self, config: RuntimeConfig) -> MarketSnapshot:
        ...
        # 🛡️ YFINANCE ANTI-THROTTLE & FALLBACK 🛡️
        data = {}
        max_retries = 3
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"  {yellow}YFinance rate-limit/error detected. Retrying in {wait_time}s...")
                    time.sleep(wait_time)
                else:
                    print(f"\n⚠️  {bold}YFinance Critical Failure:{reset} {e}")
                    print("  Switching to OFFLINE mode with minimal data snapshots.")
        ...
        return MarketSnapshot(
            ...
            generated_at=datetime.utcnow().replace(microsecond=0).isoformat() + "Z",
            volatility_news=yf.Ticker(yf_volatility).news[:5]
        )
